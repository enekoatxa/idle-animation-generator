...
def loadBvhToList(path, returnHeader = False, returnData = True, returnCounter = True, 
    ...
    if onlyPositions:
        rotationIndexes = np.arange(0, len(data[0]))
        rotationIndexes = [index for index in rotationIndexes if not (index%6==0 or (index-1)%6==0 or (index-2)%6==0)]
        for vectorIndex in range(0, len(data)):
            data[vectorIndex] = np.delete(data[vectorIndex], rotationIndexes, axis=0).copy()
            
    if onlyRotations:
        positionIndexes = np.arange(0, len(data[0]))
        positionIndexes = [index for index in positionIndexes if (index%6==0 or (index-1)%6==0 or (index-2)%6==0)]
        for vectorIndex in range(0, len(data)):
            data[vectorIndex] = np.delete(data[vectorIndex], positionIndexes, axis=0).copy()
    ...
    for root, dirs, files in os.walk(path):
        for filename in files:
            ...
            if(str(filename).split(".")[-1]=="bvh"):
                ...
                if specificSize!=-1 and id>=specificSize-1:
                    break
